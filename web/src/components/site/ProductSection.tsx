...
export function ProductSection({
  ...
  return (
    <section
      id="how"
      className="scroll-mt-24 bg-[var(--paper)] px-4 py-16 sm:px-6 md:px-10 md:py-24"
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <ol className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {howItWorks.map((step, i) => (
            <Reveal key={step.step} delay={80 + i * 90}>
              <li className="flex h-full flex-col rounded-[12px] border border-[var(--line)] bg-white/60 p-4 transition-shadow duration-300 hover:shadow-[0_12px_32px_-18px_rgba(11,11,12,0.28)] sm:p-5">
                <span className="font-data text-[11px] text-[var(--muted)]">
                  {step.step}
                </span>
                <h3 className="mt-1 text-[16px] font-semibold tracking-[-0.02em] text-[var(--ink)]">
                  {step.title}
                </h3>
