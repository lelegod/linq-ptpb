...
export default function DocsPage() {
  ...
  return (
    <PageShell>
      ...
      <ol className="mt-8 space-y-7 sm:mt-10 sm:space-y-8">
        {docsGuide.map((step, i) => (
          <li key={step.title} className="flex gap-3 sm:gap-4">
            <span className="shrink-0 font-data text-[13px] text-[var(--red)]">
      ...
      <div className="mt-10 rounded-[12px] border border-[var(--line)] bg-white px-4 py-5 sm:mt-12 sm:px-5 sm:py-6">
        ...
        <div className="mt-4 flex flex-col gap-2.5 sm:flex-row sm:flex-wrap sm:gap-3">
          <a
            href={messagesHref}
            className="inline-flex min-h-12 items-center justify-center rounded-[10px] bg-[var(--red)] px-5 py-2.5 text-[14px] font-semibold text-white hover:opacity-90"
          >
            {copy.cta}
          </a>
          <a
            href="/start"
            className="inline-flex min-h-12 items-center justify-center rounded-[10px] px-5 py-2.5 text-[14px] font-semibold ring-1 ring-[var(--line)] hover:bg-[var(--paper)]"
          >
            {copy.navGetStarted}
          </a>
        </div>
